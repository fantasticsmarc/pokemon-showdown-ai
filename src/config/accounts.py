...
def get_account_configuration(
    play_format: int, base_username: str
) -> AccountConfiguration:
    if play_format == 1:
        # Local play does not require authentication, so each bot can simply use its visible name as the account name.
        return AccountConfiguration(username=base_username, password=None)

    # Ladder play uses one shared account from config.json because only one ladder bot is created at a time in main.py.
